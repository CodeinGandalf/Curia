...
def key_callback(msg):
    ...
    global dx, dy, drot, run, dGripper, dElevator, homePose, take_map1, take_map2
    key = msg.data.lower()
...
def main():
    ...
    global take_map1, take_map2, dx, dy, drot, dGripper, dElevator, homePose, run, latest_map
    ...
    # LIDAR-Punkte als Streudiagramm
    ax.scatter(angles, distances, s=5, label='LIDAR Messpunkte')
    # Ursprung markieren (LIDAR-Position)
    ax.scatter(0, 0, color='red', s=50, label='LIDAR Position')
    # Radiusbegrenzung auf 2 Meter
    #ax.set_rmax(2.0)
    # Grid aktivieren
    ax.grid(True)
    # Titel
    ax.set_title('LIDAR-Punkte in Polarkoordinaten (Radius ≤ 2 m)', va='bottom')
    # Legende anzeigen
