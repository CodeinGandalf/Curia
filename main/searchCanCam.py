...
def find_best_can(pose_Can, camera_index):
    ...
    for contour in contours:
        ...
        if best_contour:
            ...
            cv2.putText(output_frame, f"Dose", (x, y - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

            print(f"Center=({x+w//2}, {y+h//2}), width={w}, hight={h},\
                  ratio={h/w:.2f}, probability score={best_score:.3f}")

        # Show the original and the result:
        cv2.imshow("Original", frame)
        cv2.imshow("Ergebnis", output_frame)

    # Check if the aplication should be closed:
    while True:
